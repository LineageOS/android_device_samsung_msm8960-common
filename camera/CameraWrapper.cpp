...
static int& whoCalledMe(int reset) {
    int temp = 0;
    static int processid;
    if (reset) {
        temp = android::IPCThreadState::selfOrNull()->getCallingPid();
        //Sometimes the calling PID gets set to the system process or null
        processid = (temp > 1 ? temp : processid);
        pname[0] = '\0';        
    }
    return processid;
}
...
char * camera_fixup_setparams(struct camera_device * device, const char * settings)
{
    ...
    if(pname[0] == 0) {
        ...
        if (fptr != NULL) {
            ...
            fscanf(fptr, "%s", pname);
            ALOGD("[nardshu]Caller is %s", pname);

            for(int i = 0; i < excludeListLen; i++) {
                if(strcmp(excludeList[i], pname) == 0) {
                ALOGD("[nardshu]Camera opened by excluded app %s, not enabling Samsung camcorder mode.", pname);
                exclude = 1;
                }
            }
        } else {
            ALOGD("[nardshu]Unable to open /proc/%i/cmdline", pid);
        }
        fclose(fptr);
    }
    ...
    if (exclude) {
        ...
        ALOGD("[nardshu]Enabling Samsung camcorder mode for %s", pname);
